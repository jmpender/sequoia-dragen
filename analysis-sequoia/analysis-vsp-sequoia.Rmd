...
df_vsp_expanded4 <- df_vsp_expanded4 %>%
  left_join(panel %>% select(Reporting_Name, Family_Name, Genus_Name, family_pandemic_potential, family_epidemic_potential, strand.type, taxid),
...
df_tri_spike_samples_spike.rm <- remove_spikes(df_tri_spike_samples, tri.spike)
df_samples_spike.rm <- remove_spikes(df_single_spike_samples_has_OC43, bcov.spike)
df_influent_spike.rm <- remove_spikes(df_influent_has_OC43, bcov.spike)
df_solids_spike.rm <- remove_spikes(df_solids_has_OC43, bcov.spike)

df_samples_spike.rm_direct.compare <- remove_spikes(df_single_spike_samples_has_OC43_direct.compare, bcov.spike)
df_influent_spike.rm_direct.compare <- remove_spikes(df_influent_has_OC43_direct.compare, bcov.spike)
df_solids_spike.rm_direct.compare <- remove_spikes(df_solids_has_OC43_direct.compare, bcov.spike)
...
# samples with spikes rm
df_tri_spike_samples_spike.rm_2 <- process2(df_tri_spike_samples_spike.rm)
df_samples_spike.rm_2 <- process2(df_samples_spike.rm)
df_influent_spike.rm_2 <- process2(df_influent_spike.rm)
df_solids_spike.rm_2 <- process2(df_solids_spike.rm)


# direct compare - 5 WWTPs - without spike rm
...
# direct compare - 5 WWTPs - spike rm
df_samples_spike.rm_direct.compare_2 <- process2(df_samples_spike.rm_direct.compare)
df_influent_spike.rm_direct.compare_2 <- process2(df_influent_spike.rm_direct.compare)
df_solids_spike.rm_direct.compare_2 <- process2(df_solids_spike.rm_direct.compare)


# with all viruses retained including ones that have no hits in df
# samples without spikes rm, all viruses retained
...
# samples with spikes rm
df_samples_spike.rm_2_undropped <- process2(df_samples_spike.rm, drop.absent.viruses = F)
df_influent_spike.rm_2_undropped <- process2(df_influent_spike.rm, drop.absent.viruses = F)
df_solids_spike.rm_2_undropped <- process2(df_solids_spike.rm, drop.absent.viruses = F)

```

# Optional: Consolidate to Genera and export
```{r}
summarize_by_genus <- function(df) {
  df %>%
    group_by(sample, Genus_Name) %>%
    summarize(
...
# Put all input data frames in a named list which will be used for export
dfs <- list(
  vsp_all                           = df_vsp_expanded_complete_2,   # all samples incl controls
  controls                          = df_controls_2,
  nonsamples                        = df_nonsamples_2,
  
  tri_spike_samples_inc_spikes      = df_tri_spike_samples_2,
  samples_inc_spike                 = df_single_spike_samples_has_OC43_2,
  influent_inc_spike                = df_influent_has_OC43_2,
  solids_inc_spike                  = df_solids_has_OC43_2,
  
  tri_spike_samples_spike_rm        = df_tri_spike_samples_spike.rm_2,
  samples_spike_rm                  = df_samples_spike.rm_2,
  influent_spike_rm                 = df_influent_spike.rm_2,
  solids_spike_rm                   = df_solids_spike.rm_2,
  
  samples_inc_spike_direct.compare  = df_single_spike_samples_has_OC43_direct.compare_2,
  influent_inc_spike_direct.compare = df_influent_has_OC43_direct.compare_2,
  solids_inc_spike_direct.compare   = df_solids_has_OC43_direct.compare_2,

  samples_spike_rm_direct.compare   = df_samples_spike.rm_direct.compare_2,
  influent_spike_rm_direct.compare  = df_influent_spike.rm_direct.compare_2,
  solids_spike_rm_direct.compare    = df_solids_spike.rm_direct.compare_2
)

# 3) Summarize all of them in one shot
dfs_genera <- map(dfs, summarize_by_genus)

# 4) Export function: write one JSON per sample
#    Creates a subfolder per "set" name, uses sample as filename
export_genus_jsons <- function(df, out_dir) {
  dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
  split(df, df$sample) %>%
    iwalk(function(d, sample_name) {
      # named vector: names = genus, values = rpkm
      vec <- setNames(d$rpkm, d$Genus_Name)
      write_json(as.list(vec),
                 file.path(out_dir, paste0(sample_name, ".json")),
                 pretty = TRUE, auto_unbox = TRUE)
    })
}

# 5) Export every set to its own folder
iwalk(dfs_genera, function(df, set_name) {
  export_genus_jsons(df, file.path("genera_json_files", set_name))
})

```

# Export unique sample info to Excel for overall Dragen analysis (input for dragen_report.Rmd)
```{r}
dfs <- list(
  single_spike_all.wwtp       = df_single_spike_samples_has_OC43_2,
  influent_all.wwtp           = df_influent_has_OC43_2,
  solids_all.wwtp             = df_solids_has_OC43_2,
  
  single_spike_direct.compare = df_single_spike_samples_has_OC43_direct.compare_2,
  influent_direct.compare     = df_influent_has_OC43_direct.compare_2,
  solids_direct.compare       = df_solids_has_OC43_direct.compare_2
)

# These files will be imported to dragen_report.Rmd for overall analysis
output_filenames <- c(
  single_spike_all.wwtp       = "single_spike_all.wwtp.xlsx",
  influent_all.wwtp           = "influent_all.wwtp.xlsx",
  solids_all.wwtp             = "solids_all.wwtp.xlsx",
  
  single_spike_direct.compare = "single_spike_direct.compare.xlsx",
  influent_direct.compare     = "influent_direct.compare.xlsx",
  solids_direct.compare       = "solids_direct.compare.xlsx"
)

dir.create("sample_lists", showWarnings = FALSE)
...
for (name in names(dfs)) {
  dfs[[name]] %>%
    ...
    write_xlsx(file.path("sample_lists", output_filenames[[name]]))
}
...
```{r}
df_all_sample_summary <- df_single_spike_samples %>%
    group_by(fraction, WWTP) %>%
    summarize(unique_samples_all = n_distinct(sample)) %>%
    arrange(fraction, WWTP)

df_all_viable_sample_summary <- df_single_spike_samples_has_OC43 %>%
    group_by(fraction, WWTP) %>%
    summarize(unique_samples_viable = n_distinct(sample)) %>%
    arrange(fraction, WWTP)

df_joined_sample_summary <- full_join(
  df_all_sample_summary,
  df_all_viable_sample_summary,
  by = c("fraction", "WWTP")
)

```

# BCoVscatterplot - "Is the correlation between BCoV ddPCR and sequencing RPKM strong enough for normalization, via the R^2 value? - A: No"
```{r}
...
df_bcov_influent_direct.compare <- df_influent_has_OC43_direct.compare_2 %>%
  filter(name == bcov.spike)

df_bcov_solids <- df_solids_has_OC43_2 %>%
  filter(name == bcov.spike)

df_bcov_solids_direct.compare <- df_solids_has_OC43_direct.compare_2 %>%
  filter(name == bcov.spike)

# ---- Plot BCoV RPKM for each sample ----
plot.bcov.samples <- function(df){
  p <- ggplot(df, aes(x = bcov_rpkm, y = sample)) +
    geom_point() +
    theme_minimal()
  
  return(p)
}
plot.bcov.samples(df_bcov_influent)
plot.bcov.samples(df_bcov_solids)

# ---- filter solids because some have unusually high RPKM for their group ----
df_bcov_solids_filtered <- df_bcov_solids %>%
  filter(bcov_rpkm < 1000)

df_bcov_solids_direct.compare_filtered <- df_bcov_solids_direct.compare %>%
  filter(bcov_rpkm < 1000)

# ---- Plot ddPCR ----
plot.bcov.ddpcr <- function(df, xlab, ylab.eq){
  p <- ggplot(df, aes(x = BCoV.ddpcr.recovery, y = bcov_rpkm)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE, color = "red") +
    stat_regline_equation(
      aes(label = ..eq.label..), 
      label.x = 2, label.y = ylab.eq
    ) +
    stat_cor(
...
plot.bcov.ddpcr(df_bcov_influent_direct.compare, ylab.eq=500)
plot.bcov.ddpcr(df_bcov_solids_direct.compare_filtered, ylab.eq=5)
...
df_samples_spike.rm_only.hits <- only.hits(df_samples_spike.rm_2)


plot_metrics(df_single_spike_samples_has_OC43_only.hits, output_prefix = "samples.with.spike")
plot_metrics(df_samples_spike.rm_only.hits, output_prefix = "samples.spike.rm")
...
full_grid <- expand.grid(
  name = unique(df_samples_spike.rm_processed$name),
  location = unique(df_samples_spike.rm_processed$location),
  stringsAsFactors = FALSE
)
...
df_virus_location_summary <- full_grid %>%
  ...
  relocate(
  sort(unlist(lapply(
    sort(unique(df_samples_spike.rm_processed$location)),
    function(loc) {
...
export_unique_hits_by_location(df_samples_spike.rm_processed, 
                               use_abbr = TRUE, output_file = "unique_hits_by_location.xlsx")

export_unique_hits_by_location(df_influent_spike.rm_processed, 
                               use_abbr = FALSE, output_file = "unique_hits_by_location_influent.xlsx")

export_unique_hits_by_location(df_solids_spike.rm_processed, 
...
# analysis
```{r}
namelist_influent <- unique(df_influent_spike.rm_2$name)
namelist_solids <- unique(df_solids_spike.rm_2$name)
namelist_influent_direct.compare <- unique(df_influent_spike.rm_direct.compare_2$name)
namelist_solids_direct.compare <- unique(df_solids_spike.rm_direct.compare_2$name)
...
total_rpkm_influent <- sum(df_influent_spike.rm_direct.compare_2$rpkm, na.rm = TRUE)
total_rpkm_solids <- sum(df_solids_spike.rm_direct.compare_2$rpkm, na.rm = TRUE)

total_reads_influent <- sum(df_influent_spike.rm_direct.compare_2$alignedReadCount, na.rm = TRUE)
total_reads_solids <- sum(df_solids_spike.rm_direct.compare_2$alignedReadCount, na.rm = TRUE)
...
consolidate <- function(df, consolidation.level = c("fraction", "WWTP", "WWTP_fraction")) {
  ...
  df_combined <- df_combined %>%
    left_join(panel %>% select(Reporting_Name, Genus_Name, Family_Name, family_pandemic_potential, family_epidemic_potential, strand.type, taxid),
            by = c("name" = "Reporting_Name"))
...
df_influent_by.location_spike.rm <- consolidate(df_influent_spike.rm_2, "WWTP") # spike rm
df_solids_by.location_spike.rm <- consolidate(df_solids_spike.rm_2, "WWTP") # spike rm
...
df_samples_by.fraction_spike.rm <- consolidate(df_samples_spike.rm_2, "fraction") # spike rm
...
df_samples_by.WWTP.fraction_spike.rm <- consolidate(df_samples_spike.rm_2, "WWTP_fraction") # spike rm
...
# direct compare - 5 WWTPs - spike rm
df_samples_by.fraction_direct.compare_spike.rm<- consolidate(df_samples_spike.rm_direct.compare_2, "fraction")
df_single_by.WWTP.fraction_direct.compare_spike.rm <- consolidate(df_samples_spike.rm_direct.compare_2, "WWTP_fraction")
df_influent_by.location_direct.compare_spike.rm <- consolidate(df_influent_spike.rm_direct.compare_2, "WWTP")
df_solids_by.location_direct.compare_spike.rm <- consolidate(df_solids_spike.rm_direct.compare_2, "WWTP")
...
df_influent_by.location_spike.rm_undropped <- consolidate(df_influent_spike.rm_2_undropped, "WWTP") # spike rm
df_solids_by.location_spike.rm_undropped <- consolidate(df_solids_spike.rm_2_undropped, "WWTP") # spike rm
...
df_samples_by.fraction_spike.rm_undropped <- consolidate(df_samples_spike.rm_2_undropped, "fraction") # spike rm
...
df_top13_fraction_spike.rm <- top_viruses(df_samples_by.fraction_direct.compare_spike.rm, n=13)
df_top13_influent_spike.rm <- top_viruses(df_influent_by.location_direct.compare_spike.rm, n=13)
df_top13_solids_spike.rm <- top_viruses(df_solids_by.location_direct.compare_spike.rm, n=13)
```
...
heatmap.tree <- function(df, consolidation.level = c("fraction", "WWTP", "WWTP_fraction"), 
  ...
  # df <- df_samples_by.WWTP.fraction_spike.rm
  prefix <- sub("_processed$", "", sub("^df_", "", deparse(substitute(df))))
...
heatmap.tree(df_samples_by.WWTP.fraction_spike.rm, "WWTP_fraction", "fraction.present", w=12, h=12, mid=0.0001, low=0.0001, high=1, vsp.label.loc = -1.85, tree.prop=0.65, strand.prop=0.066)
min(df_samples_by.WWTP.fraction_spike.rm$log.relabund.norm, na.rm = TRUE)
heatmap.tree(df_samples_by.WWTP.fraction_spike.rm, "WWTP_fraction", "log.relabund.norm", w=12, h=12, mid=-5.7, low=-5.7, high=0, vsp.label.loc = -1.75, tree.prop=0.65, strand.prop=0.066)

heatmap.tree(df_samples_by.fraction_spike.rm, "fraction", "fraction.present", w=8, h=12, mid=0.0001, low=0.0001, high=1, vsp.label.loc = -1.85, tree.prop=0.85, strand.prop=0.2)
min(df_samples_by.fraction_spike.rm$log.relabund.norm, na.rm = TRUE)
heatmap.tree(df_samples_by.fraction_spike.rm, "fraction", "log.relabund.norm", w=8.5, h=12, mid=-6.4, low=-6.4, high=0, vsp.label.loc = -1.75, tree.prop=0.85, strand.prop=0.2)
...
df_strand.type_by.fraction_spike.rm <- summarize.by.strand.type(df_samples_by.fraction_spike.rm, "fraction")
df_strand.type_by.fraction_spike.rm$fraction <- factor(df_strand.type_by.fraction_spike.rm$fraction,
                                                         levels = levels.fraction)

df_strand.type_influent_by.wwtp_spike.rm <- summarize.by.strand.type(df_influent_by.location_spike.rm, "WWTP")
df_strand.type_influent_by.wwtp_spike.rm$WWTP <- factor(df_strand.type_influent_by.wwtp_spike.rm$WWTP,
                                                         levels = levels.WWTP)

df_strand.type_solids_by.wwtp_spike.rm <- summarize.by.strand.type(df_solids_by.location_spike.rm, "WWTP")
df_strand.type_solids_by.wwtp_spike.rm$WWTP <- factor(df_strand.type_solids_by.wwtp_spike.rm$WWTP,
                                                         levels = levels.WWTP)


df_strand.type_by.fraction_direct.compare_spike.rm <- summarize.by.strand.type(df_samples_by.fraction_direct.compare_spike.rm, "fraction")
df_strand.type_by.fraction_direct.compare_spike.rm$fraction <- factor(df_strand.type_by.fraction_direct.compare_spike.rm$fraction,
                                                         levels = levels.fraction)

df_strand.type_influent_by.wwtp_direct.compare_spike.rm <- summarize.by.strand.type(df_influent_by.location_direct.compare_spike.rm, "WWTP")
df_strand.type_influent_by.wwtp_direct.compare_spike.rm$fraction <- factor(df_strand.type_influent_by.wwtp_direct.compare_spike.rm$WWTP,
                                                         levels = levels.WWTP)

df_strand.type_solids_by.wwtp_direct.compare_spike.rm <- summarize.by.strand.type(df_solids_by.location_direct.compare_spike.rm, "WWTP")
df_strand.type_solids_by.wwtp_direct.compare_spike.rm$fraction <- factor(df_strand.type_solids_by.wwtp_direct.compare_spike.rm$WWTP,
                                                         levels = levels.WWTP)
...
plot.strand.type(df_strand.type_by.fraction_spike.rm, "fraction")
plot.strand.type(df_strand.type_influent_by.wwtp_spike.rm, "WWTP")
plot.strand.type(df_strand.type_solids_by.wwtp_spike.rm, "WWTP")

plot.strand.type(df_strand.type_by.fraction_direct.compare_spike.rm, "fraction")
plot.strand.type(df_strand.type_influent_by.wwtp_direct.compare_spike.rm, "WWTP")
plot.strand.type(df_strand.type_solids_by.wwtp_direct.compare_spike.rm, "WWTP")
...
consolidated.plot.by.virus(df_influent_by.location_spike.rm, "WWTP", w=15, h=10)
consolidated.plot.by.virus(df_solids_by.location_spike.rm, "WWTP", w=15, h=10)

# By fraction
consolidated.plot.by.virus(df_vsp_samples_by.fraction_has.spike, "fraction", w=15, h=10)
consolidated.plot.by.virus(df_samples_by.fraction_spike.rm, "fraction", w=15, h=10)
...
# direct compare - 5 WWTPs - spike rm
consolidated.plot.by.virus(df_samples_by.fraction_direct.compare_spike.rm, "fraction", w=15, h=10)
consolidated.plot.by.virus(df_influent_by.location_direct.compare_spike.rm, "WWTP", w=15, h=10)
consolidated.plot.by.virus(df_solids_by.location_direct.compare_spike.rm, "WWTP", w=15, h=10)

# Top 13 plots
consolidated.plot.by.virus(df_top13_fraction_spike.rm, "fraction", w=10, h=7)
consolidated.plot.by.virus(df_top13_influent_spike.rm, "WWTP", w=10, h=7)
consolidated.plot.by.virus(df_top13_solids_spike.rm, "WWTP", w=10, h=7)
...
upset.plot(df_influent_by.location_spike.rm, "WWTP", w=15, h=10, t=2, b=0.5)
upset.plot(df_solids_by.location_spike.rm, "WWTP", w=15, h=10, t=2, b=0.5)

# By fraction
upset.plot(df_vsp_samples_by.fraction_has.spike, "fraction", w=18, h=10, t=2, b=0.17)
upset.plot(df_samples_by.fraction_spike.rm, "fraction", w=18, h=10, t=2, b=0.17)

# By WWTP and fraction
upset.plot(df_vsp_samples_by.WWTP.fraction_has.spike, "WWTP_fraction", w=22, h=10, t=2, b=0.6)
upset.plot(df_samples_by.WWTP.fraction_spike.rm, "WWTP_fraction", w=22, h=10, t=2, b=0.6)


```

# Genus/Family level for consolidated
```{r}
summarize_by_family_consolidated <- function(df) {
  df %>%
    group_by(WWTP, Family_Name) %>%
    summarize(
      relabund.rpkm.mean.normalized = sum(relabund.rpkm.mean.normalized, na.rm = TRUE),
      present = as.integer(any(present == 1)),
      family_pandemic_potential = first(family_pandemic_potential),
      family_epidemic_potential = first(family_epidemic_potential),
      .groups = "drop"
    )
}

df_influent_by.location_spike.rm_family                <- summarize_by_family_consolidated(df_influent_by.location_spike.rm)
df_influent_by.location_direct.compare_spike.rm_family <- summarize_by_family_consolidated(df_influent_by.location_direct.compare_spike.rm)
df_solids_by.location_spike.rm_family                  <- summarize_by_family_consolidated(df_solids_by.location_spike.rm)
df_solids_by.location_direct.compare_spike.rm_family   <- summarize_by_family_consolidated(df_solids_by.location_direct.compare_spike.rm)
...
wwtp_to_hpi_city <- tibble(
  WWTP = c("Modesto", "Turlock", "Woodland", "Merced", "Los Banos", 
           "City of Davis", "UC Davis", "Esparto", "Winters"),
  hpi.city = c("Modesto", "Turlock", "Woodland", "Merced", "Los Banos", 
               "Davis", "Davis", "Esparto", "Winters")
)


df_influent_means <- read_excel("../../outputs-sequoia/vsp/dragen_report_means/influent_means.xlsx")
df_solids_means   <- read_excel("../../outputs-sequoia/vsp/dragen_report_means/solids_means.xlsx")

# Filter for the means for some sources from the Dragen overall report
filter.dragen.means <- function(df){
  df_filter <- df %>%
    group_by(WWTP) %>%
    summarize(
      AMR = relabund_mean_normalized[source_type == "Untargeted AMR"],
      VSP_Target = relabund_mean_normalized[source_type == "VSP Target"],
      .groups = "drop"
    )
  return(df_filter)
}

means_inf_select <- filter.dragen.means(df_influent_means)
means_sol_select <- filter.dragen.means(df_solids_means)

################################
hpi.loc.summary <- function(df, family_df, means_df){
  df_summary <- df %>%
    group_by(WWTP) %>%
    summarize(
      total.viruses.present = sum(as.numeric(present), na.rm = TRUE),
      .groups = "drop"
    )
  
   df_summary_family <- family_df %>%
    group_by(WWTP) %>%
    summarize(
      total.vsp.families.present = sum(as.numeric(present), na.rm = TRUE),

      # counts of families present with High pandemic / epidemic potential
      high.pandemic.families = sum(as.integer(present == 1 & family_pandemic_potential == "High"), na.rm = TRUE),
      high.epidemic.families = sum(as.integer(present == 1 & family_epidemic_potential == "High"), na.rm = TRUE),

      # families present that are High in either category (union, not double-counted)
      any.high.risk.families = sum(as.integer(
        present == 1 & (family_pandemic_potential == "High" | family_epidemic_potential == "High")
      ), na.rm = TRUE),
      .groups = "drop"
    )
  
df_summary <- df_summary %>%
  left_join(wwtp_to_hpi_city, by = "WWTP") %>%
  left_join(df_summary_family, by = "WWTP") %>%
  left_join(means_df, by = "WWTP") %>%
  left_join(hpi, by = c("hpi.city" = "name"))

  return(df_summary)
}
################################

summary_inf_all_wwtps      <- hpi.loc.summary(df_influent_by.location_spike.rm,
                                              df_influent_by.location_spike.rm_family, means_inf_select)
summary_inf_direct_compare <- hpi.loc.summary(df_influent_by.location_direct.compare_spike.rm,
                                              df_influent_by.location_direct.compare_spike.rm_family, means_inf_select)
summary_sol_all_wwtps      <- hpi.loc.summary(df_solids_by.location_spike.rm,
                                              df_solids_by.location_spike.rm_family, means_sol_select)
summary_sol_direct_compare <- hpi.loc.summary(df_solids_by.location_direct.compare_spike.rm,
                                              df_solids_by.location_direct.compare_spike.rm_family, means_sol_select)

hpi_labels <- c(
  population = "Population",
  hpi.score_percentile = "Healthy Places Index (Percentile)",
  economic.overall_percentile = "Economic Index (Percentile)",
  above.poverty_percentile = "Above Poverty (Percentile)",
  employed_percentile = "Employed (Percentile)",
  percapitaincome_percentile = "Per Capita Income (Percentile)",
  education.overall_percentile = "Education (Percentile)",
  bachelorsedorhigher_percentile = "Bachelor's Ed or Higher (Percentile)",
  housing.overall_percentile = "Housing (Percentile)",
  housing.habitability_percentile = "Housing Habitability (Percentile)",
  uncrowded_percentile = "Uncrowded (Percentile)",
  clean.enviro.overall_percentile = "Clean Environment (Percentile)",
  h2o.contam_percentile = "Water Contamination (Percentile)",
  insurance.overall_percentile = "Insurance (Percentile)",
  insured.adults_percentile = "Insured Adults (Percentile)",
  social_percentile = "Social (Percentile)",
  race.ethnicity.diversity_percentile = "Race/Ethnicity Diversity Index (Percentile)"
)

dragen_labels <- c(
  total.viruses.present = "Total Number of VSP Viruses Present",
  AMR = "Untargeted AMR Relative Abundance of Reads",
  VSP_Target = "Relative Abundance (Reads) of VSP Targets",
  total.vsp.families.present = "Total Number of Families Present (VSP panel)",
  high.pandemic.families = "High Pandemic-Potential Families (count)",
  high.epidemic.families = "High Epidemic-Potential Families (count)",
  any.high.risk.families = "Any High-Risk Families (count)"
)

hpi.plot <- function(df, xval, yval){
  # Get axis labels from hpi_labels and dragen_labels
  xlab <- hpi_labels[[xval]] %||% xval
  ylab <- dragen_labels[[yval]] %||% yval
  
  df <- df %>%
    mutate(
      across(all_of(c(xval, yval)), ~ as.numeric(.))
    )
  
  p <- ggplot(df, aes(x = .data[[xval]], y = .data[[yval]])) +
    geom_point(size = 3) +
    ...
    geom_text(aes(label = hpi.city), vjust = -0.7, size = 3.5) +
...
# Named list of dataframes and corresponding output subdirectories
plot_specs <- list(
  inf_all_wwtps      = summary_inf_all_wwtps,
  inf_direct_compare = summary_inf_direct_compare,
  sol_all_wwtps      = summary_sol_all_wwtps,
  sol_direct_compare = summary_sol_direct_compare
)

# What to plot, where to save it, and filename suffix
plot_y_specs <- tribble(
  ~yval,                         ~subdir_base,                                  ~suffix,
  "total.viruses.present",       "1_total_viruses_present",                     "total.viruses.present",
  "VSP_Target",                  "2_vsp_targets",                               "vsp.target",
  "AMR",                         "3_amr_untargeted",                            "amr",
  "total.vsp.families.present",  "4_total_families_present",                    "total.families.present",
  "high.pandemic.families",      "5_total_high_risk_pandemic_families",         "high.pandemic.families",
  "high.epidemic.families",      "6_total_high_risk_epidemic_families",         "high.epidemic.families",
  "any.high.risk.families",      "7_total_high_risk_families_either",           "any.high.risk.families"
)

# Make HPI vs virus data plots! Warning: generates hundreds of graphs.
# One pass over dataset groups, one pass over Y specs, one pass over HPI x's
pwalk(
  .l = list(df = plot_specs, subdir = names(plot_specs)),
  .f = function(df, subdir) {
    pmap(plot_y_specs, function(yval, subdir_base, suffix) {
      outdir <- file.path("hpi_plots", subdir_base, subdir)
      dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

      walk(names(hpi_labels), ~ ggsave(
        filename = file.path(outdir, paste0(.x, "_vs_", suffix, ".pdf")),
        plot = hpi.plot(df = df, xval = .x, yval = yval),
...
plots_vsp_tri.spike.samples_tri.spike.rm <- plot_abundance(df_tri_spike_samples_spike.rm_processed, w=8, h=7, n=1)
...
plots_vsp_influent_bcov.spike.rm_by_location <- plot_consolidated_abundance(df_influent_by.location_spike.rm2, 
                                                                 "WWTP", w=15, h=10, n=2) # spike rm
plots_vsp_solids_by_location <- plot_consolidated_abundance(df_solids_by.location_has.spike, 
                                            "WWTP", w=15, h=10, n=2) # spike intact
plots_vsp_solids_bcov.spike.rm_by_location <- plot_consolidated_abundance(df_solids_by.location_spike.rm2, 
                                                               "WWTP", w=15, h=10, n=2) # spike rm
...
plots_vsp_samples_bcov.spike.rm_by_fraction <- plot_consolidated_abundance(df_samples_by.fraction_spike.rm2, 
                                                            "fraction", w=15, h=10, n=3) # spike rm
...
scatter.single.viruses(df_solids_spike.rm_2, virus="Human adenovirus F", type="Solids")
scatter.single.viruses(df_solids_has_OC43_2, virus=bcov.spike, type="Solids")

scatter.single.viruses(df_influent_spike.rm_2, virus="Human adenovirus F", type="Influent")
...
df_sars_influent <- df_influent_spike.rm_2_undropped %>% filter(name == sars)
df_sars_solids <- df_solids_spike.rm_2_undropped %>% filter(name == sars)
